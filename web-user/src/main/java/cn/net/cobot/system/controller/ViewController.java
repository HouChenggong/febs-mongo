...
import cn.net.cobot.common.authentication.ShiroHelper;
...
public class ViewController extends BaseController {
    ...
    public String index(Model model) {
        ...
        UserMongo user = super.getCurrentUserMongo();
        UserMongo currentUserDetail = userMongoService.findByName(user.getUsername());
//        currentUserDetail.setPassword("It's a secret");
    ...
    private void resolveUserModel(String username, Model model, Boolean transform) {
        UserMongo user = userMongoService.findByName(username);
        model.addAttribute("user", user);
        if (transform) {
            String ssex = user.getSex();
            if (UserMongo.SEX_MALE.equals(ssex)) {
                user.setSex("男");
            } else if (UserMongo.SEX_FEMALE.equals(ssex)) {
                user.setSex("女");
            } else {
                user.setSex("保密");
            }
